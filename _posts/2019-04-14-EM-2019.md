...
$$
= \frac{P(X|Z=1,\theta)P(Z=1|\theta)}{\pi p^x(1-p)^{1-x} + (1-\pi)q^x(1-q)^{1-x}}
$$

$$
= \frac{p^x(1-p)^{1-x}}{\pi p^x(1-p)^{1-x} + (1-\pi)q^x(1-q)^{1-x}}
$$
M-step:
...
$$
\theta^{t+1} = arg\max_\theta \int_Z P(Z|X,\theta^t)logP(X,Z|\theta)dz \\
= \sum_{i=1}^N \sum_ZP(Z|x_i,\theta^t)logP(x_i,Z|\theta) 
$$
