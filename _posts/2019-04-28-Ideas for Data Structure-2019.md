...
一般矩形由给定的两个点确定，一个点为左下角的坐标，一个点为右上角的坐标
...
- 第二个在第一个上边
  - 若第二个矩形第一个点的横坐标大于第一个右上角的横坐标
- 第二个在第一个右边
  - 若第二个矩形左下角点的横坐标大于第一个点右上角的横坐标
- 第二个在第一个下边
  - 第二个矩形右上角的纵坐标小于第一个矩形左下角纵坐标
- 第二个在第一个左边
  - 第二个矩形右上角横坐标小于第一个矩形左下角横坐标



<img src="https://github.com/joseph-mutu/Pics/raw/master/data%20structure/rectancle.png" style="zoom:50%;" />

#### 树的前序中序后序迭代遍历

三种遍历可以使用同一种框架进行

从根节点开始，给定每个节点一个状态，`V,NV` 其中 `V` 表示曾经访问过，`NV` 表示第一次访问
...
给定一个字符串，求在各种限制条件下的最大收益

该系列问题可用一种框架完成，核心在于对所有`状态` 以及`选择`的枚举
...
- `dp[day][k][0]` 的定义为：在当前天数，最大交易次数为 K 且当前不持有股票的时候所能达到的最大收益

- `dp[day][k][1]` 为：在当前天数，最大交易次数为 K 且当前不持有股票的时候所能达到的最大收益

**转移方程**
$$
dp[day][k][0] = max(dp[day-1][k][0],dp[day-1][k][1] + price[day])\\
\space \\
当前最大收益可以由前一天不买，或者前一天手中股票在今天售出得到
\space\\
\space \\
dp[day][k][1] = max(dp[day-1][k][1],dp[day-1][k-1][0] - price[day]) \\
\space \\
当前持有股票的最大收益可以有由前一天状态顺延，或者前一天没有股票今天购买股票得到
$$
最终要对每种状态进行枚举

**枚举**

在枚举时，需要考虑到 `base case`，天数`day`从 0 开始，最大交易次数 `k` 从  1 开始

则需要考虑以下状态

```python
dp[-1][0][0] = 0
dp[-1][0][1] = float('-inf') # 天数为 -1 不可能持有股票
```
